public class Communities {
    ...
    public static double getPM25Level(double[][] data, int community) {
        ...
        // pm25 level is in column 14
        // data[community][14]

        return data[community][HLT_PM2_5]; // replace this line
    }
    ...
    public static double highestPopulationWithHealthIssue(double[][] data, char healthIssue) {
        ...
        if(healthIssue == 'A'){
            ...
            column = HLT_HEART_DISEASE;
        }

        for (int i = 0; i < data.length; i++) {
            if(data[i][column] > num){
                num = data[i][column];
            }
        }
    ...
    public static int numberOfCommunities(double[][] data, int columnIndex, double percentageThreshold,
        ...
        for (int i = 0; i < data.length; i++) {
            percentage = data[i][columnIndex]*100;
            if(isDisadvantage){
                if((percentage >= percentageThreshold) && (data[i][ECON_DISADVANTAGED] == 1) ){
                    count++;
                }
            } else {
                if(percentage >= percentageThreshold&& (data[i][ECON_DISADVANTAGED] == 0) ){
                    count++;
                }
            }
    ...
    public static double largestRacialDemographic(double[][] data, int community) {
        ...
        for (int i = 0; i <= 7; i++) {
            if(data[community][i] > largest){
                largest=data[community][i];
            }

        }

        return largest * 100; // replace this line
    }
    ...
    public static int mostPopulatedDisadvantagedWithLowIncome(double[][] data) {
        ...
        double population=0;
        for (int i = 0; i < data.length; i++) {
            if((data[i][ECON_DISADVANTAGED] == 1) && (data[i][ECON_LOW_INCOME]== 1)){
                if(data[i][TOTAL_POPULATION] > population){
                    population = i;
                }
            }
        }

        return (int)population; // replace this line
    }
